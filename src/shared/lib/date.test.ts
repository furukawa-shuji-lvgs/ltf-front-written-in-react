...
describe(formatDate, () => {
  ...
  it("フォーマット指定子を含まない文字列はそのまま返すこと", () => {
    expect.hasAssertions();
    const date = new Date(2026, 6, 3);
    expect(formatDate(date, "こんにちは")).toBe("こんにちは");
  });

  it("同じ指定子が複数含まれる場合にすべて置換されること", () => {
    expect.hasAssertions();
    const date = new Date(2026, 6, 3);
