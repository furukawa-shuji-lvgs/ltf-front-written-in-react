 ...
 * ルート情報のプレーンな表現
 *
 * 移行メモ: 移行元は Nuxt の useRoute() の戻り値を受け取っていたが、 Next では usePathname() / useSearchParams() /
 * useParams() などから組み立てたプレーンな値を受け取るように API を変更した
 */
...
/**
 * 非 root ページで Next/Nuxt 互換 route 情報が root 扱いになった場合に、 リクエスト URL から復元できる値だけを補正する。
 *
 * @returns 無効な非ルートパスを補正したルート情報。
 */
export function safeRouteForNonRootPage(
  ...
  requestUrl: URL,
  getRequiredParams: () => readonly string[],
  ...
  const fallbackParams = (original: RouteInfo["params"]) => {
    const missingParams = getRequiredParams().filter(
      (parameter) => original[parameter] === undefined || original[parameter] === "",
    );
